def update_stores_payroll_table(payroll_tables_cursor, payroll_tables_conn):
    ...
    for worker_id, worker_info in payroll_dict.items():
        ...
        if workplace_info["WorkplaceType"] == "Warehouse":
            continue

        elif workplace_info["WorkplaceType"] == "Store":

            store_id = workplace_info["StoreID"]

            elements_list = [ worker_id, store_id ]

            query1 = "INSERT INTO StoreWorkers (WorkerID, StoreID)"
